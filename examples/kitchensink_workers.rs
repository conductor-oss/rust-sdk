...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let process_task = |task_name: &str, task: &conductor::models::Task| {
        ...
        output.insert("source".to_owned(), json!(task_name));
        output.insert("processed".to_owned(), json!(true));
        ...
        let odd_even = std::time::SystemTime::now()
            ...
            .as_nanos()
            % 2;
        output.insert("oddEven".to_owned(), json!(odd_even));
        ...
        output.insert("complexOutput".to_owned(), complex_data);

        // Logic specific to kitchensink workflow
        if task_name == "task_1" {
            output.insert("mod".to_owned(), json!(1));
            output.insert("oddEven".to_owned(), json!(0));
            ...
            output.insert("dynamicTasks".to_owned(), json!(dynamic_tasks));
            output.insert("inputs".to_owned(), json!(dynamic_inputs));
        }
