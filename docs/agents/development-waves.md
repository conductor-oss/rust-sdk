...
- [ ] **`AgentRuntime::resume(execution_id, agent)`** — reattach to an existing execution after a process restart: fetch the workflow, extract the worker domain from `task_to_domain`, re-register local tool workers, return a working `AgentHandle`. `src/agents/runtime.rs`/`src/agents/handle.rs`.
