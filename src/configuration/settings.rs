...
impl Configuration {
    ...
    pub fn from_env() -> Self {
        ...
        let debug =
            env::var("CONDUCTOR_DEBUG").is_ok_and(|v| v.to_lowercase() == "true" || v == "1");
...
mod tests {
    ...
    fn test_with_auth() {
        ...
        assert_eq!(config.auth_key, Some("key123".to_owned()));
        assert_eq!(config.auth_secret, Some("secret456".to_owned()));
    }
    ...
    fn test_token_needs_refresh() {
        ...
        // Add token
        config.update_token("test-token".to_owned());
