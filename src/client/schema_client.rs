//! Schema client for managing schema definitions
...
impl SchemaClient {
    ...
    pub async fn register_schema(&self, schema: &SchemaDef) -> Result<()> {
        let path = "/schema";
        self.api.post_no_response(path, schema).await
    }
    ...
    pub async fn get_schema(&self, schema_name: &str, version: i32) -> Result<SchemaDef> {
        ...
        self.api
            .get_with_params(&path, &[("version", &version.to_string())])
            .await
    }
    ...
    pub async fn delete_schema(&self, schema_name: &str, version: i32) -> Result<()> {
        let path = format!("/schema/{}", schema_name);
        self.api
            .delete_with_params(&path, &[("version", &version.to_string())])
            .await
    }
