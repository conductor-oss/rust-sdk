...
///
/// Requires the `agents` Cargo feature. The function must be async and take exactly one
/// parameter: a struct implementing `JsonSchema + DeserializeOwned`. Its JSON schema is
/// generated automatically via `conductor::schema::generate_schema`.
///
/// # Usage
///
/// ```rust,ignore
...
fn generate_tool(args: ToolArgs, input_fn: ItemFn) -> syn::Result<TokenStream2> {
    ...
    if fn_inputs.len() != 1 {
        return Err(syn::Error::new_spanned(
            fn_inputs,
            "tool function must take exactly one parameter: a struct implementing \
             `JsonSchema + DeserializeOwned`",
        ));
    }
    let (arg_pat, arg_ty) = match fn_inputs
        .first()
        .expect("checked fn_inputs.len() == 1 above")
    {
